...
async fn run_migrations(pool: &SqlitePool) -> ZenResult<()> {
    ...
    // Normalized execution traces are the durable source for Run Inspector and
    // replay. `messages.steps_json` remains as a compatibility projection for
    // older history/imports while new checkpoints write one bounded row per
    // trace event here.
    sqlx::query(
        r#"
    ...
    // rows without a risky all-at-once migration.
    queries::migrate_legacy_trace_rows(pool).await?;
