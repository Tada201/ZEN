...
0.2. **⚠️ TEMPORARY — Big Migration In Progress**: The `src-tauri` backend is under the Cargo-workspace migration defined in [BIG_MIGRATION.md](BIG_MIGRATION.md). Before any backend change, check the current phase there; never stack phases and always end a phase green (gate suite, BIG_MIGRATION.md §5). **Always cross-check the frozen pre-migration snapshot in [Zen_rs_old/](Zen_rs_old/)** when original structure, line numbers, or behavior are in doubt. Remove this rule at Phase 14 once `migration/complete` is tagged.
