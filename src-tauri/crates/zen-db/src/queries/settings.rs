...
pub async fn count_settings(pool: &SqlitePool) -> ZenResult<i64> {
    Ok(sqlx::query("SELECT COUNT(*) AS count FROM settings").fetch_one(pool).await.map_err(crate::db_err)?.get::<i64, _>("count"))
}
...
pub async fn clear_settings(pool: &SqlitePool) -> ZenResult<()> {
    sqlx::query("DELETE FROM settings").execute(pool).await.map_err(crate::db_err)?;
    Ok(())
}
