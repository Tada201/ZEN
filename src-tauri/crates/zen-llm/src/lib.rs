//! zen-llm - LLM provider clients, streaming and registry for the Zen backend
//! workspace (BIG_MIGRATION.md Phase 7).
//!
//! Pure HTTP/SSE: no tauri, no DB. Settings/secrets flow through the
//! `zen-core` store ports consumed by `registry::ProviderRegistry`.
pub mod anthropic;
pub mod lmstudio;
pub mod ollama;
...
use zen_core::{ChatMessage, ChatResponse, ModelInfo, ProviderConfig};
