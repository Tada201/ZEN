//! Unified tool discovery manager (from `src/tools/manager.rs`, Phase 5).
//! Wraps the agent-tool registry and the canonical catalog registry:
//! listing, info, existence checks, and permission management. The runner
//! still executes tools through the service layer; this manager is the
//! discovery layer (tool_list / tool_info meta-tools) and the central
//! authority for tool permissions. Pure: no tauri, no app state.
...
use zen_security::policy::{PermissionDefault, ToolPermissionRules, ToolPermissions};

use crate::agent_tool::AgentTool;
use crate::capability::{tool_aliases, tool_status};
use crate::registry::{default_tool_risk, AgentToolRegistry, ToolRegistry};
...
/// Unified tool manager that wraps both the agent-tool registry and the
/// canonical catalog registry. Provides listing, info, existence checks,
/// and permission management. The Runner still uses the agent-tool
/// registry directly for execution; this manager is the discovery layer
/// (tool_list / tool_info meta-tools) and the central authority for tool
/// permissions.
pub struct ToolManager<A> {
    v1: Arc<RwLock<AgentToolRegistry<A>>>,
    v2: Arc<RwLock<ToolRegistry<A>>>,
...
impl<A: Send + Sync + 'static> ToolManager<A> {
    pub fn new(v1: Arc<RwLock<AgentToolRegistry<A>>>, v2: Arc<RwLock<ToolRegistry<A>>>) -> Self {
        Self {
            v1,
            v2,
    ...
    /// and the catalog ToolRegistry (used by the Runner for per-call checks).
    ///
    ...
    /// being migrated to the catalog registry: there is still one
    /// discovery/validation catalog, even when the concrete executor is still
    /// an AgentTool.
    pub async fn sync_legacy_tool_definitions(&self) {
        let mut legacy_tools: Vec<Arc<dyn AgentTool<A>>> = Vec::new();
    ...
    /// List/search available tools. This includes lazy metadata, so lazy tools are
    ...
    pub async fn list_allowed_matching(
        ...
        // Lazy metadata from the agent-tool registry. This is the
        // authoritative discovery source for lazy tools; do not limit
        // discovery to already loaded tools.
        {
            let v1_guard = self.v1.read().await;
            if let Some(lazy) = v1_guard.lazy_source() {
                for meta in lazy.metadata() {
    ...
    pub async fn list_metadata(&self) -> Vec<ToolMetadata> {
        ...
        // Agent-tool registry (lazy metadata source)
        {
            let v1_guard = self.v1.read().await;
            if let Some(lazy) = v1_guard.lazy_source() {
                for meta in lazy.metadata() {
        ...
        // Catalog registry tools (Tool trait registry — use definitions for risk_level)
        {
            let v2_guard = self.v2.read().await;
            ...
            for def in v2_defs {
                ...
                if seen.insert(def.name.clone()) {
                    ...
                    // Merge with catalog definitions for supplemental schema or risk data
    ...
    pub async fn get_info(&self, id: &str) -> Option<ToolSchema> {
        // Try the agent-tool registry first
        {
            let v1_guard = self.v1.read().await;
