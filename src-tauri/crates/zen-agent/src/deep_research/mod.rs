...
pub use types::DeepResearchParams;

use serde_json::json;
use tracing::{error, info};

use crate::event_bus::{AgentEvent, ChatDonePayload};
use zen_db::queries;
...
pub async fn run_deep_research(params: DeepResearchParams<'_>) {
    ...
    // Phase 6 seam: every emit goes through the EventSink port
    // (byte-identical to the former host emit path).
