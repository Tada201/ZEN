...
impl ContextMiddleware for SystemPromptMiddleware {
    ...
    async fn enrich(&self, ctx: &mut EnrichmentContext) -> ZenResult<()> {
        use crate::runner::helpers::estimate_tokens;
        ...
        let mut head = String::with_capacity(
            crate::prompt_safety::SAFETY_PREAMBLE.len() + existing.len() + 256,
        );
        head.push_str(crate::prompt_safety::SAFETY_PREAMBLE);
        ...
        ctx.record_section(
            ...
            crate::prompt_safety::SAFETY_PREAMBLE.to_string(),
        ...
        if ctx.tools_enabled {
            {
                ...
                let inventory_block = zen_mcp::McpDiscoveryService::prompt_block(&inventory);
            ...
            .any(|t| t == "graph_session")
        {
            ...
            ctx.try_push_section(ContextSectionId::GraphSession, &mut remaining, "Supported: sin, cos, tan, sqrt, abs, ln, log10, exp, floor, ceil, and named variables.\n");
                        ...
                        // Inject current session state if available. Port seam: the app-side
            // GraphSessionSource snapshots the canvas map; None when the session
            // is absent or the map lock is contended (best-effort, as before).
...
",
        ...
        let meta_tools: Vec<zen_tools::ToolInfo> = if ctx.tools_enabled && !direct_board_agent {
            zen_tools::meta_tool_definitions()
        } else {
            Vec::new()
        };
        ...
        if !meta_tools.is_empty() {
            ...
            if ctx
                .authorized_tool_ids
                .iter()
                .any(|t| zen_mcp::client::is_external_tool_name(t))
            {
