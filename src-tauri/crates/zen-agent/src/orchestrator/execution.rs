...
use crate::event_bus::{AgentEvent, ChatChunkFirstPayload, ChatChunkPayload};
use crate::runner;
use crate::runner::actions::{ActionEmitParams, ActionPersistParams};
use crate::task::Task;
use crate::types::{ActionMeta, AgentResponse, MessageKind, SpawnMeta};
...
impl Orchestrator {
    ...
    pub(crate) async fn execute_task_with_agent(
        ...
        let mut resolved = crate::child_runner::resolve_agent(
            &self.agent_registry,
            agent_id,
            Some(model),
            Some(model),
        ...
        let mut runner = crate::child_runner::build_child_runner(
            crate::child_runner::ChildRunnerParams {
                ctx: &self.ctx,
        ...
        runner = runner.with_memory_scope(crate::child_runner::subagent_memory_scope(
            agent_id,
            &task.description,
        ));
        ...
        let emit_subagent_step = |status: &str,
                                  ...
                                  error: Option<String>,
                                  duration_ms: u64| {
            AgentEvent::SubagentStep(crate::event_bus::SubagentStepPayload {
