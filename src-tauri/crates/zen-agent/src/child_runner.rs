...
pub fn resolve_adhoc_agent(
    ...
    let agent = Agent {
        ...
        model_tier: crate::types::ModelTier::default(),
    };
...
pub fn build_child_messages_from_handoff(handoff: &HandoffContext) -> Vec<ChatMessage> {
...
pub struct ChildRunnerParams<'a> {
    /// Parent context: carries the tool catalog/execution ports and db pool
    /// the child inherits. Phase 11: replaces the former host-handle param,
    /// whose only use was managed-state lookups for values already on this
    /// context.
...
pub fn build_child_runner(params: ChildRunnerParams<'_>) -> Result<Runner> {
    ...
    let parent = Runner::new(ctx.clone(), agent_registry, hook_registry)
...
pub async fn inject_workspace_agents_md(
    workspace_folder: &Arc<tokio::sync::RwLock<std::path::PathBuf>>,
    resolved: &mut ResolvedAgent,
) {
    if !resolved.inject_agents_md {
        return;
    }
    let workspace = workspace_folder.read().await.clone();
...
pub fn subagent_memory_scope(agent_id: &str, task: &str) -> String {
