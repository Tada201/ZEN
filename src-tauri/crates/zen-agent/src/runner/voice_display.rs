...
use crate::child_runner;
use crate::ports::BoardPort;
...
impl Runner {
    pub(super) fn spawn_voice_display_agent(
        ...
        tokio::spawn(async move {
            ...
            // Port seam: the app-side BoardPort filters `board:update` by
            // chat id and flips the flag; the guard unlistens on drop
            // (dropped explicitly at the same point the former code called
            // `app.unlisten`).
            let board_watch = ctx.board.watch_board_updates(
                synthetic_chat_id.clone(),
                board_updated_listener,
            );
            ...
            if !board_updated.load(Ordering::Acquire) && !token.is_cancelled() {
                ...
                result = match json_result {
                    ...
                    .await
                    {
                        Ok(()) => Ok(crate::types::AgentResponse {
                            content: None,
                            final_answer: None,
                            tool_calls: Vec::new(),
                            reasoning: None,
...
async fn execute_deterministic_board_fallback(
    ...
    if request.contains("youtube") || request.contains("video") {
        ...
        return board
            .run_board_operation(chat_id.to_string(), operation, depth, None, token)
            .await
            .map(|_| ());
    }
