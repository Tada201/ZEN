...
use crate::middleware::{EnrichmentContext, MiddlewareChain};
use crate::types::*;
...
use crate::context_breakdown::compute_context_breakdown;
...
impl Runner {
    ...
    pub async fn run(
        ...
        config: zen_llm::ChatRequestConfig,
        token: CancellationToken,
    ) -> Result<crate::types::AgentResponse, anyhow::Error> {
        ...
        loop {
            ...
            if should_emit_iteration_status(self.depth) {
                ...
                self.emit(AgentEvent::AgentChunk(
                    crate::event_bus::AgentChunkPayload {
                        chat_id: chat_id.clone(),
            ...
            if should_emit_iteration_status(self.depth) {
                ...
                self.emit(crate::event_bus::AgentEvent::ContextBreakdown(
                    breakdown,
                ));
            }
            ...
            {
                {
                    ...
                    if let Some(bd) = updated {
                        self.emit(crate::event_bus::AgentEvent::ContextBreakdown(bd));
                    }
                }
            }
            ...
            let mut raw_calls: Vec<zen_db::models::ToolCall> = Vec::new();
            ...
            let models_tool_calls: Vec<zen_db::models::ToolCall> = tool_calls
                .iter()
                .map(|tc| zen_db::models::ToolCall {
                    id: tc.id.clone(),
                    name: tc.name.clone(),
                    args: tc.args.clone(),
                })
