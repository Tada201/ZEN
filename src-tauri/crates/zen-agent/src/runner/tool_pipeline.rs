...
pub(super) async fn preprocess_tool_calls(
    tool_manager: &Arc<dyn ToolPipelinePort>,
    tool_calls: &[ToolCall],
...
/// see `crate::cache::ttl_for_tool` for the allowlist.
pub(super) fn should_read_cache(tool_name: &str) -> bool {
    crate::cache::ttl_for_tool(tool_name).is_some()
}
...
pub(super) fn should_write_cache(tool_name: &str, is_error: bool) -> bool {
    !is_error && crate::cache::ttl_for_tool(tool_name).is_some()
}
