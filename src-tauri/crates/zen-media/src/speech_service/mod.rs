...
use crate::runtime_resource::{kill_pid_sync, RuntimeResources};
use serde::Serialize;
...
impl SpeechService {
    ...
    fn resolved_whisper_binary(
        &self,
    ) -> Result<crate::runtime_resource::ResolvedBinary, String> {
        self.runtime
    ...
    pub async fn download_model(&self, model_name: &str) -> Result<ModelFileStatus, String> {
        ...
        let response = crate::http::model_download_http_client()
            .get(&url)
            .send()
            .await
