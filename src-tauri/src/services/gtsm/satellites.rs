use super::types::{Satellite, GtsmStreamMessage};
...
pub async fn fetch_satellites(tx: &Sender<String>) -> Result<Vec<Satellite>> {
    let cache_lock = TLE_CACHE.get_or_init(|| RwLock::new(TleCache {
    ...
    if needs_fetch {
        ...
        if !status.is_success() {
            let err_body = response_res.text().await.unwrap_or_else(|_| "No body".to_string());
        ...
        tracing::info!("TLE data received ({} bytes), parsing {} lines...", response.len(), line_count);
        ...
        for i in (0..lines.len()).step_by(3) {
            ...
            let line1 = lines[i + 1];
            let line2 = lines[i + 2];
            if let Ok(els) = Elements::from_tle(Some(name.clone()), line1.as_bytes(), line2.as_bytes()) {
    ...
    for (name, elements) in &cache.elements {
            if let Ok(constants) = Constants::from_elements(&elements) {
                let now = Utc::now();
                let t_since = (now.timestamp() as f64 - elements.datetime.and_utc().timestamp() as f64) / 60.0;
                ...
                if let Ok(prediction) = constants.propagate(t_since) {
                    ...
                    let sat = Satellite {
                        ...
                        lon,
                        alt: alt * 1000.0,
                        velocity: prediction.velocity.iter().map(|&x| x * x).sum::<f64>().sqrt(),
                    };
                    ...
                    if batch.len() >= 250 {
                        if let Ok(json) = serde_json::to_string(&GtsmStreamMessage::SatelliteBatch(batch.clone())) {
                            let _ = tx.send(json);
                        }
                        batch.clear();
    ...
    tracing::info!("Satellite broadcast complete. Sent {} satellites", sent_count);
    Ok(all_results)
}
...
fn eci_to_lla(position: [f64; 3], time: DateTime<Utc>) -> (f64, f64, f64) {
    ...
    let mut gmst = 280.46061837 + 360.98564736629 * (jd - 2451545.0) + 0.000387933 * t * t - t * t * t / 38710000.0;
    gmst %= 360.0;
