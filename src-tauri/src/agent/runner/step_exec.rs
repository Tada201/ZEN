...
use crate::agent::event_bus::{AgentEvent, ChatChunkPayload, ChatDonePayload};
...
impl Runner {
    ...
    pub(super) async fn preload_skill_fragments(
        ...
        Vec<crate::agent::skills::SkillInstructionsFragment>,
        Option<std::path::PathBuf>,
    ) {
        let mut preloaded_skill_fragments: Vec<crate::agent::skills::SkillInstructionsFragment> =
            Vec::new();
    ...
    pub(super) async fn process_tool_results(
        ...
        for (tool_call, result) in tool_calls.iter().zip(results.iter()) {
            ...
            let safe_content =
                crate::agent::prompt_safety::wrap_tool_result(&tool_call.name, &content_str);
            conversation.push(ChatMessage {
                role: "tool".to_string(),
