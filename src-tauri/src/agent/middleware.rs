...
impl ContextMiddleware for SystemPromptMiddleware {
    ...
    async fn enrich(&self, ctx: &mut EnrichmentContext) -> ZenResult<()> {
        ...
        if !meta_tools.is_empty() {
            ...
            ctx.system_content.push_str("- **Correct Tool Call Format**: You MUST invoke meta-tools exactly by their names (`tool_list`, `tool_info`, `tool_exec`). Do NOT append or prefix native API strings (e.g., do NOT call `call_functions_tool_list_...` or similar wrappers). Use only the exact names defined in this protocol.\n");
