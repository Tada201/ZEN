...
impl AgentTool for RouteTool {
    fn id(&self) -> &str { "calculate_route" }
    ...
    async fn run(
        ...
        let start_coords = if let Some(origin_name) = args.origin {
            let results = crate::services::gtsm::geocoding::search(&origin_name, 1).await
                .map_err(|e| anyhow!("Failed to geocode origin '{}': {}", origin_name, e))?;
            let first = results.first().ok_or_else(|| anyhow!("Origin '{}' not found", origin_name))?;
            [first.lat, first.lon]
        } else {
            [
                args.start_lat.ok_or_else(|| anyhow!("origin or start_lat required"))?,
                args.start_lon.ok_or_else(|| anyhow!("origin or start_lon required"))?,
            ]
        };

        // ── Resolve Destination ──
        let end_coords = if let Some(dest_name) = args.destination {
            let results = crate::services::gtsm::geocoding::search(&dest_name, 1).await
                .map_err(|e| anyhow!("Failed to geocode destination '{}': {}", dest_name, e))?;
            let first = results.first().ok_or_else(|| anyhow!("Destination '{}' not found", dest_name))?;
            [first.lat, first.lon]
        } else {
            [
                args.end_lat.ok_or_else(|| anyhow!("destination or end_lat required"))?,
                args.end_lon.ok_or_else(|| anyhow!("destination or end_lon required"))?,
            ]
        };

        let profile = args.profile.unwrap_or(crate::services::gtsm::types::RoutingProfile::Car);

        // ── Get AppState for keys and DB ──
        let state = app.state::<crate::commands::AppState>();
        let pool = state.db().await.map_err(|e| anyhow::anyhow!("DB Init failed: {}", e))?;

        // Fetch API keys from settings
        let here_key = crate::db::queries::get_setting(&pool, "here_api_key").await?.unwrap_or_default();
        let google_key = crate::db::queries::get_setting(&pool, "google_maps_api_key").await?.unwrap_or_default();
        ...
        let route = crate::services::gtsm::navigation::compute_route(
            ...
            Some(&pool)
        ).await.map_err(|e| anyhow!("Route calculation failed: {}", e))?;
        ...
        let traffic_info = route.traffic_duration_s.map(|t| format!(" (including {:.0}m traffic)", t / 60.0)).unwrap_or_default();
...
impl AgentTool for GeocodeTool {
    ...
    async fn run(
        ...
        let results = crate::services::gtsm::geocoding::search(query, limit).await
...
impl AgentTool for ReverseGeocodeTool {
    fn id(&self) -> &str { "reverse_geocode" }
    ...
    async fn run(
        ...
        let lat = input["lat"].as_f64().ok_or_else(|| anyhow!("lat required"))?;
        let lon = input["lon"].as_f64().ok_or_else(|| anyhow!("lon required"))?;

        let result = crate::services::gtsm::geocoding::reverse(lat, lon).await
