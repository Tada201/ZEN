...
pub async fn run_deep_research(
    ...
    if !urls_to_fetch.is_empty() {
        emit_step(
            &format!("Reading {} sources...", urls_to_fetch.len()),
            "running",
            &message_id,
        );
    ...
    let on_chunk = Box::new(move |chunk: crate::llm::LlmChunk| {
        let text = match chunk {
            ...
            crate::llm::LlmChunk::ToolCallReady { .. } => return,
        };
        if !text.is_empty() {
    ...
    let res = match llm_provider
        .chat_stream(&model, llm_messages, None, config, on_chunk, token.clone())
        .await
    {
        Ok(res) => res,
        Err(e) => {
    ...
    .await
    {
        ...
        emit_step(
            "Failed to save final research report to database",
            "error",
            &message_id,
        );
        return;
