...
impl ToolManager {
    ...
    ///   tools.global-default      → "confirm" | "always_allow" | "always_deny"
    ///   tools.yolo-mode           → "true" | "false"
    ///   tools.auto-approve-low-risk → "true" | "false"
    ///   tools.permission.{id}.default → "inherit" | "allow" | "deny" | "confirm"
    ///   tools.permission.{id}.allow-patterns  → newline-separated regex patterns
    ///   tools.permission.{id}.deny-patterns   → newline-separated regex patterns
    ///   tools.permission.{id}.confirm-patterns → newline-separated regex patterns
    pub fn build_permissions(settings: &HashMap<String, String>) -> ToolPermissions {
        let global_default = match settings.get("tools.global-default").map(|s| s.as_str()) {
            Some("always_allow") => PermissionDefault::AlwaysAllow,
            Some("always_deny") => PermissionDefault::AlwaysDeny,
            _ => PermissionDefault::Confirm,
        };

        let yolo_mode = settings.get("tools.yolo-mode").map(|s| s == "true").unwrap_or(false);
        let auto_approve_low_risk = settings.get("tools.auto-approve-low-risk").map(|s| s == "true").unwrap_or(false);

        // Collect per-tool overrides by scanning keys
        let mut tool_overrides: HashMap<String, ToolPermissionRules> = HashMap::new();

        for (key, value) in settings {
            // Match keys like: tools.permission.{id}.default
            //                      tools.permission.{id}.allow-patterns
            if key.starts_with("tools.permission.") {
                ...
                if let Some(dot_pos) = remainder.find('.') {
                    ...
                    let rules = tool_overrides.entry(tool_id).or_insert_with(ToolPermissionRules::default);

                    match field {
                        "default" => {
                            // Replace only the `default` field, preserving any already-collected patterns.
                            // `std::mem::take` swaps `rules` with `Default::default()` (empty vecs)
                            // and returns the old value, letting us build a new struct with the
                            // new `default` but all existing patterns carried over.
                        ...
                        "allow-patterns" => {
                            rules.always_allow = value.lines().filter(|l| !l.trim().is_empty()).map(|l| l.trim().to_string()).collect();
                        }
                        "deny-patterns" => {
                            rules.always_deny = value.lines().filter(|l| !l.trim().is_empty()).map(|l| l.trim().to_string()).collect();
                        }
                        "confirm-patterns" => {
                            rules.always_confirm = value.lines().filter(|l| !l.trim().is_empty()).map(|l| l.trim().to_string()).collect();
                        }
                        _ => {}
                    }
                }
