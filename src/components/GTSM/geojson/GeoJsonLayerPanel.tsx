...
export const GeoJsonLayerPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-1.5 p-3 border border-border bg-card font-mono text-xs w-full max-h-[220px] overflow-y-auto">
      <div className="flex items-center justify-between text-cyan-400 border-b border-border pb-1 mb-1">
      ...
      {layers.length === 0 ? (
        <div className="text-center py-4 text-muted-foreground text-[9px] uppercase tracking-wider">
        ...
        <div className="flex flex-col gap-1">
          {layers.map((layer) => {
            ...
            return (
              <div 
                key={layer.id} 
                className="border border-border bg-card p-1.5 rounded flex flex-col gap-1 hover:border-border transition-colors"
              >
                {/* Header Row */}
