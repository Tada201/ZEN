  ...
  return (
    ...
    >
      <div className="space-y-3">
        {camera.isDemo ? <p className="border border-warning bg-warning/10 px-3 py-2 text-[11px] leading-5 text-warning">Playback diagnostic only. This is not represented as a live public camera.</p> : null}
        <div className="aspect-video border border-border bg-background">
          {!playback ? (
            <button type="button" disabled={isResolving} onClick={() => void startPreview()} className="flex h-full w-full flex-col items-center justify-center gap-2 text-foreground hover:bg-muted disabled:cursor-wait disabled:opacity-60">
        ...
        {playbackError ? <p role="alert" className="border border-rose-400 bg-rose-400/10 px-3 py-2 text-[11px] leading-5 text-rose-100">{playbackError}</p> : null}
  ...
  return (
    <>
      <div className="pointer-events-auto relative font-mono">
        <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex h-7 items-center gap-1.5 border border-border bg-card px-2 text-[10px] font-medium text-foreground shadow-lg hover:bg-muted">
        ...
        {open ? (
          <section className="absolute right-0 top-8 z-40 w-[min(92vw,340px)] border border-border bg-card shadow-2xl shadow-lg">
            ...
            <div className="max-h-[min(52vh,420px)] overflow-y-auto p-2">
              ...
              {cameras.length === 0 ? <p className="px-2 py-5 text-center text-[11px] text-muted-foreground">No vetted camera feeds are configured.</p> : cameras.map((camera) => (
                <button key={camera.id} type="button" onClick={() => inspect(camera)} className="w-full border-b border-border px-2 py-2.5 text-left last:border-b-0 hover:bg-muted">
              ...
              <button type="button" onClick={() => setLocalPreviewOpen(true)} className="mt-2 flex w-full items-center gap-2 border border-dashed border-border px-2 py-2.5 text-left text-[10px] text-foreground hover:bg-muted">
