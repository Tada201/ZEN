...
function CameraPreviewDialog({ camera, onClose }: { camera: MapCameraCatalogEntry; onClose: () => void }) {
  ...
  return (
    <AppDialog
      ...
      footer={<button type="button" onClick={onClose} className="border border-white/10 px-3 py-2 text-[11px] text-zinc-300 hover:bg-white/[0.06]">Close</button>}
    >
      <div className="space-y-3">
        ...
        <div className="aspect-video border border-white/10 bg-black">
          {!playback ? (
            <button type="button" disabled={isResolving} onClick={() => void startPreview()} className="flex h-full w-full flex-col items-center justify-center gap-2 text-zinc-300 hover:bg-white/[0.04] disabled:cursor-wait disabled:opacity-60">
        ...
        <div className="flex items-center justify-between gap-3 border-t border-white/10 pt-3 text-[10px] leading-5 text-zinc-500">
        ...
        {camera.attribution || camera.termsUrl ? <p className="text-[10px] leading-5 text-zinc-500">{camera.attribution ?? camera.operator}{camera.termsUrl ? <><span> · </span><a href={camera.termsUrl} target="_blank" rel="noreferrer" className="text-primary hover:underline">Terms</a></> : null}</p> : null}
      </div>
    </AppDialog>
  );
}
  ...
  return (
    <>
      <div className="pointer-events-auto relative font-mono">
        <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex h-7 items-center gap-1.5 border border-white/15 bg-black/45 px-2 text-[10px] font-medium text-zinc-100 shadow-lg shadow-black/30 backdrop-blur-md hover:bg-black/60">
          <WorkbenchIcon name="solar:videocamera-record-bold" size={14} className="text-primary" />
          <span>Cameras</span>
          <span className="text-zinc-500">{cameras.length}</span>
        </button>
        {open ? (
          <section className="absolute right-0 top-8 z-40 w-[min(92vw,340px)] border border-white/15 bg-black/85 shadow-2xl shadow-black/60 backdrop-blur-xl">
            <header className="flex items-center justify-between border-b border-white/10 px-3 py-2.5">
              <div><p className="text-[11px] font-medium text-zinc-100">Camera catalog</p><p className="mt-0.5 text-[10px] text-zinc-400">Vetted sources only{checkedAt ? ` · checked ${formatCheckedAt(checkedAt)}` : ''}</p></div>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => void refreshCatalog()} className="p-1 text-zinc-400 hover:text-white" aria-label="Refresh camera catalog"><WorkbenchIcon name="solar:refresh-linear" size={15} /></button>
                <button type="button" onClick={() => setOpen(false)} className="p-1 text-zinc-400 hover:text-white" aria-label="Close camera catalog"><WorkbenchIcon name="solar:close-circle-linear" size={16} /></button>
              </div>
            </header>
            <div className="max-h-[min(52vh,420px)] overflow-y-auto p-2">
              {sources.map((source) => <div key={source.id} className="border-b border-white/10 px-2 py-2 last:border-b-0"><div className="flex items-center justify-between gap-2 text-[10px]"><span className="truncate text-zinc-300">{source.label}</span><span className={source.status === 'available' ? 'text-emerald-300' : source.status === 'unavailable' ? 'text-rose-300' : 'text-zinc-500'}>{sourceStatusLabel[source.status]}</span></div>{source.detail ? <p className="mt-1 text-[9px] leading-4 text-zinc-500">{source.detail}</p> : null}</div>)}
              {cameras.length === 0 ? <p className="px-2 py-5 text-center text-[11px] text-zinc-500">No vetted camera feeds are configured.</p> : cameras.map((camera) => (
                <button key={camera.id} type="button" onClick={() => inspect(camera)} className="w-full border-b border-white/10 px-2 py-2.5 text-left last:border-b-0 hover:bg-white/[0.045]">
