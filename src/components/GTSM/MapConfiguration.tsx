...
function SecretField({ label, detail, value, present, onChange, onClear }: { label: string; detail: string; value: string; present: boolean; onChange: (value: string) => void; onClear: () => void }) {
  return <div className="border border-border bg-muted p-3">
    ...
    <div className="mt-3 flex gap-2"><input type="password" autoComplete="off" value={value} onChange={(event) => onChange(event.target.value)} placeholder={present ? 'Enter a replacement value' : 'Paste key'} className="h-8 min-w-0 flex-1 border border-border bg-card px-2 text-[11px] text-foreground outline-none focus:border-primary" />{present ? <button type="button" onClick={onClear} className="border border-rose-400 px-2 text-[10px] text-rose-200 hover:bg-rose-400/10">Clear</button> : null}</div>
  </div>;
}
...
export function MapConfiguration() {
  ...
  return <div className="space-y-6">
    ...
    {notice ? <p role="status" className="border border-border bg-muted px-3 py-2 text-[11px] text-foreground">{notice}</p> : null}
