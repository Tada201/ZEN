  ...
  return (
    <div className="pointer-events-auto relative font-mono">
      <button
        ...
        className="flex h-7 items-center gap-1.5 border border-border bg-background/45 px-2 text-[10px] font-medium text-foreground shadow-lg shadow-black/30 backdrop-blur-md transition-colors hover:bg-background/60"
      >
      ...
      {open && (
        <section className="absolute right-0 top-8 z-40 flex w-[min(92vw,340px)] max-h-[min(72vh,620px)] flex-col overflow-hidden border border-border bg-background/70 shadow-2xl shadow-black/60 backdrop-blur-xl">
            ...
            {tab === 'sources' && (
              <div className="space-y-2">
                ...
                {connectors.map((source) => {
                  ...
                  return (
                    <div key={source.id} className="border border-border bg-muted/30 px-3 py-2.5">
                ...
                {cameraSources.map((source) => (
                  <div key={source.id} className="border border-border bg-muted/30 px-3 py-2.5">
                    ...
                    {source.detail && <p className={`mt-2 border-l pl-2 text-[9px] leading-4 ${source.status === 'unavailable' ? 'border-rose-400/50 text-rose-200' : 'border-border text-muted-foreground'}`}>{source.detail}</p>}
                  </div>
                ))}
              </div>
            )}
            ...
            {tab === 'imports' && (
              <div className="space-y-3">
                ...
                <div className="border border-border bg-muted/30 p-3">
                  ...
                  {cameraImportMessage ? <p className="mt-2 border-l border-emerald-400/50 pl-2 text-[10px] leading-4 text-success">{cameraImportMessage}</p> : null}
