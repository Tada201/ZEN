...
export function ChartCard({ data }: { data: ChartData }) {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-lg flex flex-col">
      ...
      <div className="w-full h-48 relative flex items-center justify-center bg-card rounded-xl border border-border p-3 overflow-hidden">
        {datasets.length === 0 || labels.length === 0 ? (
          <span className="text-[10px] font-mono text-muted-foreground">No data provided</span>
        ) : chartType === "pie" ? (
          // SVG Pie/Donut Chart
              ...
              return datasets[0].data.map((val, idx) => {
                ...
                return (
                  <path
                    ...
                    d={pathData}
                    fill={color}
                    className="hover:opacity-95 transition-opacity cursor-pointer border border-border"
                  >
      ...
      {/* Legends & Labels */}
      <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 mt-3 text-[10px] font-mono text-primary-foreground border-t border-border pt-2.5">
        {chartType === "pie"
