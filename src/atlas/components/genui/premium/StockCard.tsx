    ...
    <div className="rounded-2xl border border-border/[0.08] bg-background/40 backdrop-blur-md p-5 shadow-lg max-w-md">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-[10px] text-primary-foreground/40 uppercase tracking-wider">{data.companyName || 'Company Inc.'}</p>
        </div>
      ...
      {/* Stats Row */}
      <div className="grid grid-cols-2 gap-4 border-t border-border/[0.06] pt-3 text-[11px] font-mono">
        <div>
          <span className="text-primary-foreground/30 block uppercase tracking-wider">Market Cap</span>
          <span className="text-primary-foreground/80 font-bold">{data.marketCap || 'N/A'}</span>
        </div>
        <div>
          <span className="text-primary-foreground/30 block uppercase tracking-wider">Volume</span>
          <span className="text-primary-foreground/80 font-bold">{data.volume || 'N/A'}</span>
        </div>
      </div>

      {/* 52-Week Range Bar */}
      <div className="mt-3.5 border-t border-border/[0.06] pt-3 text-[10px]">
        <div className="flex justify-between text-primary-foreground/40 font-mono mb-1">
          <span>52W Low: ${data.low52 || '0.00'}</span>
          <span>52W High: ${data.high52 || '0.00'}</span>
        </div>
        <div className="w-full h-1 bg-card/10 rounded-full relative overflow-hidden">
