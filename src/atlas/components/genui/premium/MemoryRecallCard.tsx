    ...
    <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-md p-5 shadow-lg flex flex-col">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <h3 className="text-sm font-semibold text-white leading-tight font-mono truncate max-w-[220px]" title={query}>
              "{query}"
            </h3>
          </div>
        </div>
      ...
      <div className="space-y-2.5 flex-1 max-h-64 overflow-y-auto pr-1">
        {chunks.map((chunk, idx) => (
          <div
            key={idx}
            className="flex flex-col p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] relative group/chunk"
          >
            <div className="flex items-center justify-between gap-2 mb-1.5 text-[9px] font-mono text-white/30">
              <span className="flex items-center gap-1.5 text-white/50">
                <FileText className="w-2.5 h-2.5 text-white/20" /> {chunk.source}
              </span>
            ...
            <p className="text-[11px] text-white/80 leading-relaxed italic line-clamp-3 select-all">
              "{chunk.text}"
            </p>
          </div>
        ))}
