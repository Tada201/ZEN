  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-md p-5 shadow-lg flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-baseline gap-2.5">
            <h3 className="text-lg font-bold text-white tracking-tight">{word}</h3>
            {phonetic && (
      ...
      <div className="space-y-4 flex-1">
        {entries.map((entry, idx) => (
          <div key={idx} className="flex flex-col">
            ...
            <p className="text-[12px] text-white/80 leading-relaxed font-sans pl-1">
              {entry.definition}
            </p>

            {entry.examples && entry.examples.length > 0 && (
              <ul className="pl-4 mt-1.5 space-y-1 list-disc text-white/40 text-[11px] italic">
      ...
      {(etymology || synonyms.length > 0) && (
        <div className="mt-5 pt-3.5 border-t border-white/[0.06] space-y-2 text-[10px] font-mono">
          {etymology && (
            <div className="text-white/40 leading-relaxed">
              <span className="text-white/20 uppercase tracking-wider block mb-0.5">Etymology</span>
              {etymology}
            </div>
          )}
          {synonyms.length > 0 && (
            <div className="text-white/40">
              <span className="text-white/20 uppercase tracking-wider block mb-1">Synonyms</span>
              <div className="flex flex-wrap gap-1.5 mt-0.5">
                {synonyms.map((syn) => (
                  <span
                    key={syn}
                    className="text-[9px] px-2 py-0.5 rounded-full bg-white/[0.04] text-white/60 border border-white/[0.02]"
                  >
                    {syn}
                  </span>
                ))}
