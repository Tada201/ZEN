  ...
  return (
    <div className="genui-card-surface w-full max-w-none min-w-0 rounded-2xl border border-border bg-card p-5 shadow-lg flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-baseline gap-2.5">
            ...
            {phonetic && (
              <span className="text-[11px] font-mono text-primary">{phonetic}</span>
            )}
          </div>
        ...
        {entries.map((entry, idx) => (
          <div key={idx} className="flex flex-col">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-primary/10 border border-primary text-primary">
                {entry.pos}
              </span>
              <div className="flex-1 h-px bg-muted" />
            </div>

            <p className="text-[12px] text-primary-foreground leading-relaxed font-sans pl-1">
              {entry.definition}
            </p>

            {entry.examples && entry.examples.length > 0 && (
              <ul className="pl-4 mt-1.5 space-y-1 list-disc text-muted-foreground text-[11px] italic">
      ...
      {(etymology || synonyms.length > 0) && (
        <div className="mt-5 pt-3.5 border-t border-border space-y-2 text-[10px] font-mono">
          {etymology && (
            <div className="text-muted-foreground leading-relaxed">
              <span className="text-muted-foreground uppercase tracking-wider block mb-0.5">Etymology</span>
              {etymology}
            </div>
          )}
          {synonyms.length > 0 && (
            <div className="text-muted-foreground">
              <span className="text-muted-foreground uppercase tracking-wider block mb-1">Synonyms</span>
