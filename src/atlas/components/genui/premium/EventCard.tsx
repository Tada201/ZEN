    ...
    <div className="rounded-2xl border border-border/[0.08] bg-background/40 backdrop-blur-md p-5 shadow-lg max-w-md flex gap-4">
      ...
      <div className="flex-grow flex flex-col justify-between min-w-0">
        <div>
          ...
          <div className="flex items-center gap-1.5 text-xs text-primary-foreground/50 mt-1.5 font-medium">
            <MapPin size={11} className="shrink-0 text-primary-foreground/40" />
            <span className="truncate">{data.venue || 'Venue Location'}</span>
          </div>
          <div className="flex items-center gap-1.5 text-xs text-primary-foreground/50 mt-1 font-medium">
            <Clock size={11} className="shrink-0 text-primary-foreground/40" />
            <span>{data.time || '7:00 PM'}</span>
          </div>
        </div>
        <div className="flex items-center justify-between border-t border-border/[0.06] pt-3 mt-3">
          <span className="text-xs font-mono font-bold text-primary-foreground/40">{data.price || 'Free'}</span>
