...
export function TimelineCard({ data }: { data: TimelineData }) {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-md p-5 shadow-lg">
      <h3 className="text-sm font-semibold text-white mb-4 tracking-tight">{title}</h3>

      <div className="relative pl-6 space-y-5 before:absolute before:left-2 before:top-1.5 before:h-[calc(100%-14px)] before:w-px before:bg-white/[0.08]">
        {events.map((event, idx) => {
          const status = (event.status || "upcoming").toLowerCase();
          
          let statusColor = "text-white/20 border-white/10 bg-white/5";
          let statusTextClass = "text-white/60";
          ...
          if (status === "done" || status === "complete" || status === "completed") {
            ...
            statusTextClass = "text-white font-semibold";
          }
