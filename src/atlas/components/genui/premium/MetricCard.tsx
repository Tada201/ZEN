...
export function MetricCard({ data }: { data: any }) {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-black/40 backdrop-blur-md p-5 shadow-lg w-full max-w-[240px] text-center">
      ...
      {trend != null && (
        <div className={`flex items-center justify-center gap-1 mt-2 text-xs font-medium ${
          trendUp ? 'text-emerald-400' : trendDown ? 'text-rose-400' : 'text-white/30'
        }`}>
          {trendUp ? <TrendingUp size={14} /> : trendDown ? <TrendingDown size={14} /> : <Minus size={14} />}
          <span>{typeof trend === 'number' ? `${trend > 0 ? '+' : ''}${trend}%` : trend}</span>
        </div>
      )}
      {subtitle && <p className="text-[10px] text-white/30 mt-1.5">{subtitle}</p>}
    </div>
  );
}
