...
export function BoardEquation({ block }: { block: VoiceStageEquationBlock }) {
  ...
  return (
    <div className="flex min-h-full min-w-0 flex-col items-center justify-center overflow-hidden p-4">
      {block.title && <div className="mb-3 text-xs font-semibold text-primary-foreground/70">{block.title}</div>}
      <div
        className="max-w-full text-primary-foreground [&_.katex-display]:m-0 [&_.katex-display]:max-w-full [&_.katex-display]:overflow-hidden"
...
export function BoardChart({ block }: { block: VoiceStageChartBlock }) {
  ...
  const options = useMemo(() => ({
    ...
    plugins: { legend: { display: Boolean(block.title), labels: { color: "hsl(var(--foreground) / 0.7)" } } },
    scales: {
      x: { ticks: { color: "hsl(var(--foreground) / 0.55)" }, grid: { color: "hsl(var(--foreground) / 0.06)" } },
      y: { ticks: { color: "hsl(var(--foreground) / 0.55)" }, grid: { color: "hsl(var(--foreground) / 0.08)" } },
    },
  }), [block.title]);
...
export function BoardDiagram({ block }: { block: VoiceStageKrokiBlock }) {
  ...
  return (
    <div className="h-full overflow-hidden rounded-lg border border-border/[0.04]">
      <div className="px-3 py-1.5 text-[10px] font-semibold uppercase text-primary-foreground/45">{block.title || block.diagram}</div>
      <pre className="whitespace-pre-wrap p-3 font-mono text-[11px] text-primary-foreground/60">{block.content}</pre>
    </div>
  );
}
