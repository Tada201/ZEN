...
export function VoiceDiagnosticsPanel({
    ...
    return (
        <div className="w-full bg-[#05060b]/80 border border-border/5 rounded-xl p-4 overflow-hidden text-[9px] font-mono text-muted-foreground flex flex-col gap-3 z-10">
            <div className="flex justify-between items-center text-muted-foreground border-b border-border/5 pb-1">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                ...
                <div className="flex justify-between"><span>LATENCY</span><span className="text-primary-foreground">{tokensPerSec ? `${(1000 / tokensPerSec).toFixed(0)}ms` : '24ms'}</span></div>
            </div>
            {whisperBackendDetail && (
                <div className="truncate rounded border border-border/5 bg-card/[0.02] px-2 py-1 text-[9px] text-muted-foreground">
                    {whisperBackendDetail}
                </div>
            )}
            <div className="h-px bg-card/5 my-1" />
