...
export function summarizeExecutionSteps(steps: GroupedAssistantStep[]): ExecutionSummary | undefined {
  let agents = 0;
  let handoffs = 0;
  let approvals = 0;
  let tools = 0;
  let parallelToolGroups = 0;
  let running = 0;
  let completed = 0;
  let errors = 0;
  ...
  const addPhaseLabel = (phase: unknown) => {
    ...
    const label = {
      ...
      llm_invoked: "model",
      agent_invoked: "agent",
      orchestrator_invoked: "orchestrator",
      tool_batch_planned: "tools",
      tool_batch_running: "tools running",
    }[phase] || phase.replace(/_/g, " ");
    if (!phaseLabels.includes(label)) phaseLabels.push(label);
  };

  const addAgentLabel = (label: unknown) => {
    if (typeof label === "string" && label.trim()) {
      agentLabels.add(label.trim());
    }
  };
  ...
  steps.forEach((step) => {
    if (step.type === "tool-group") {
      ...
      if (step.toolCalls.length > 1) parallelToolGroups += 1;
      step.toolCalls.forEach((tool) => {
        addAgentLabel(tool.agentName || tool.agentId);
        if (tool.status === "running") activeToolNames.add(tool.name);
        if (tool.status === "awaiting_approval") approvalToolNames.add(tool.name);
        if (tool.status === "error") errors += 1;
        else if (tool.status === "completed") completed += 1;
        else running += 1;
      });
      return;
    }
    ...
    if (kind === "chat_status") addPhaseLabel(step.metadata?.phase);
    if (kind === "agent_spawn" || kind === "agent_complete") {
      agents += 1;
      addAgentLabel(step.metadata?.spawn?.childAgent || step.metadata?.agentName || step.metadata?.agentId);
    }
    if (kind === "agent_handoff") {
      addAgentLabel(step.metadata?.handoff?.fromAgent);
      addAgentLabel(step.metadata?.handoff?.toAgent);
    }
    if (step.metadata?.assignedTo) addAgentLabel(step.metadata.assignedTo);
    if (kind === "agent_handoff") handoffs += 1;
    if (kind === "approval_request") approvals += 1;
    if (
      kind === "agent_spawn" ||
      kind === "agent_complete" ||
      kind === "agent_handoff" ||
      ...
      kind === "task_complexity_analyzed" ||
      kind.startsWith("task_") ||
      kind.startsWith("workflow_")
    ) {
      if (step.status === "error") errors += 1;
      else if (step.status === "completed") completed += 1;
      else running += 1;
    }
  });

  const total = running + completed + errors;
  if (total === 0 && phaseLabels.length === 0) return undefined;

  const phaseSummary = phaseLabels.length > 0 ? phaseLabels.slice(0, 6).join(" -> ") : undefined;
  const parts = [
    phaseSummary ? `pipeline ${phaseSummary}` : "",
    agentLabels.size > 0 ? `agents ${Array.from(agentLabels).slice(0, 4).join(", ")}` : "",
    parallelToolGroups > 0 ? `${parallelToolGroups} parallel batch${parallelToolGroups === 1 ? "" : "es"}` : "",
    agents > 0 ? `${agents} agent event${agents === 1 ? "" : "s"}` : "",
    tools > 0 ? `${tools} tool call${tools === 1 ? "" : "s"}` : "",
    activeToolNames.size > 0 ? `active ${Array.from(activeToolNames).slice(0, 3).join(", ")}` : "",
    approvalToolNames.size > 0 ? `waiting approval ${Array.from(approvalToolNames).slice(0, 3).join(", ")}` : "",
    handoffs > 0 ? `${handoffs} handoff${handoffs === 1 ? "" : "s"}` : "",
    approvals > 0 ? `${approvals} approval request${approvals === 1 ? "" : "s"}` : "",
  ].filter(Boolean);

  if (total === 0 && phaseLabels.length > 0) {
    const phaseTotal = phaseLabels.length;
    const phaseCompleted = Math.max(0, phaseTotal - 1);
    return {
      ...
      errors: 0,
      total: phaseTotal,
      progressPercent: Math.round((phaseCompleted / phaseTotal) * 100),
      phaseSummary,
      phaseLabels,
    };
  }
  ...
  return {
    label: tools > 1 || agents > 1 ? "Agent execution" : tools > 0 ? "Tool execution" : "Agent progress",
    detail: parts.join(" / ") || `${total} execution event${total === 1 ? "" : "s"}`,
    running,
    completed,
    errors,
    total,
    progressPercent: Math.round(((completed + errors) / total) * 100),
    phaseSummary,
    phaseLabels,
  };
}
