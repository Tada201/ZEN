...
const processManager = read('src-tauri/src/services/process_manager.rs');
...
assert(panel.includes('terminalApi.kill(chatId, tab.id)'), 'Unmounted terminal panels must clean up live sessions.');
assert(panel.includes("from '@/components/ui/AppDialog'"), 'Opening a terminal must use the shared in-app confirmation dialog.');
assert(panel.includes('terminalApi.requestApproval(chatId)'), 'The shared confirmation dialog must request a session-scoped backend approval token.');
